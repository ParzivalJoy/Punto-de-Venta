...
class ImageDownload(Resource):
    def get(self, filename):
        ...
        try:
            ...
            return send_from_directory(os.getenv("UPLOADED_EMOJIS_DEST", './static/img/openmoji-72x72-color'),
                               filename)

class EmojiList(Resource):
    def get(self):
        onlyfiles = [f for f in listdir(os.getenv("UPLOADED_EMOJIS_DEST", './static/img/openmoji-72x72-color')) if isfile(join(os.getenv("UPLOADED_EMOJIS_DEST", './static/img/openmoji-72x72-color'), f))]
        # print(onlyfiles)
